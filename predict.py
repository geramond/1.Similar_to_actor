...
def predict_actress(image: np.array,
    ...
    if len(face_bounding_boxes) != 1:
        ...
        predict_name = list(dict_labels_women.keys())[list(dict_labels_women.values()).index(predict)]
        predict_proba = model.predict_proba([face_enc])[0][predict][0]

        frame_proba = pd.DataFrame()
        frame_proba['actress'] = list(dict_labels_women.keys())
...
def predict_actor(image: np.array,
    ...
    if len(face_bounding_boxes) != 1:
        ...
        # Predict actor
        predict = model.predict([face_enc])
        predict_name = list(dict_labels_men.keys())[list(dict_labels_men.values()).index(predict)]
        predict_proba = model.predict_proba([face_enc])[0][predict][0]

        frame_proba = pd.DataFrame()
        frame_proba['actor'] = list(dict_labels_men.keys())
        frame_proba['score'] = model.predict_proba([face_enc])[0]
